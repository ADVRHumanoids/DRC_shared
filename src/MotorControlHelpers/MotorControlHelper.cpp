...
{
    getInterface(poly, posCtrl_p,   "IPositionControl");
    getInterface(poly, posCtrl2_p,  "IPositionControl2");

    getInterface(poly, velCtrl_p,   "IVelocityControl");
    getInterface(poly, velCtrl2_p,  "IVelocityControl2");

    getInterface(poly, encoder_p,   "IEncodersTimed");
    getInterface(poly, amplif_p,    "IAmplifierControl");

    getInterface(poly, controlLim_p,"IControlLimits2");
    getInterface(poly, axisInfo_p,  "IAxisInfo");

    getInterface(poly, stamp_p,     "IPreciselyTimed");
    getInterface(poly, calib2_p,    "IControlCalibration2");

    getInterface(poly, trqCtrl_p,   "ITorqueControl");
    getInterface(poly, impCtrl2_p,  "IImpedanceControl");

    getInterface(poly, ctrlMode_p,  "IControlMode");
    getInterface(poly, openLoop_p,  "IOpenLoopControl");
    getInterface(poly, directCtrl_p,"IPositionDirect");
    return true;
}
...
bool MotorControlHelper::open(yarp::os::Searchable *config)
{
    ...
    ConstString robotName = config->check("robot", Value("coman"),"Getting robot name").asString().c_str();
    ...
    ConstString locPortName = config->check("local",Value("comanBasic"),"Output port name").asString();
